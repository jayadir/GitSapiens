...
export default function Chat({ id, issueId = null, issueName = null }) {
  const { attachments: attachmentState, removeAttachment } = useAttachmentsStore();
  ...
  const ChatMessage = ({ message, isLatestAi }) => (
    <div className={`flex items-start gap-3 mb-4 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`relative max-w-[75%] rounded-2xl p-4 ${
        message.role === "user" 
          ? "bg-blue-600 text-white" 
          : "bg-black border border-gray-700 text-gray-100"
      }`}>
        {message.role==="user"? message.content:<MDEditor.Markdown 
  ...
  return (
    <div>
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="w-[85vw] h-[95vh] max-w-none p-0 bg-gray-900 border-gray-800">
          <DialogHeader className="px-6 py-4 border-b border-gray-800">
            <DialogTitle className="text-gray-100">Chat {issueName && `- ${issueName}`}</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex flex-col max-h-60 overflow-y-auto mb-4 border border-gray-800 rounded-md p-3 bg-gray-800/50">
            {chatHistory.length > 0 ? (
              chatHistory.slice(-3).map((msg, index) => (
                <div 
                  key={index} 
                  className={`mb-2 ${msg.role === "user" ? "text-blue-400" : "text-gray-300"}`}
                >
